...
func dartPathField(field string) string {
...
func (r *splitDartRenderer) renderDecoder(typ *wireType) error {
	...
	case kindMap:
		...
		r.line("    final item = fgbDecode%d(rawValue, bridge, '$path[$key]');", typ.Elem.ID)
...
func (r *splitDartRenderer) renderEncoder(typ *wireType) error {
	...
	r.line("  if (depth > 64) {")
	r.line("    throw FormatException('$path: value nesting exceeds 64 levels (cyclic reference?)');")
	r.line("  }")
	...
	case kindStruct:
		...
		for _, field := range typ.Struct.allFields() {
			r.line("    %s: fgbEncode%d(value.%s, bridge, %s, depth + 1),", strconv.Quote(field.WireName), field.Type.ID, field.DartName, dartPathField(field.WireName))
		}
		r.line("  };")
	case kindOpaque:
		r.line("  if (value == null) return null;")
